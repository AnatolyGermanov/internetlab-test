...
const Slider: FC<SliderProps> = ({ reviews }) => {
    ...
    const slicedReviews = reviews.slice(
        startSlide,
        startSlide + visibleReviewsCount
    );

    const moveSlider = (step: number) => {
        setStartSlide((prev) => prev + step);
    };

    useEffect(() => {
        calcVisibleReviewsCount();
        window.addEventListener('resize', calcVisibleReviewsCount);

        return () => {
            window.removeEventListener('resize', calcVisibleReviewsCount);
        };
    }, []);
    ...
    return (
        <div className={styles.slider}>
            <button
                className={clsx(styles.slider__button, 'hidden-tablet')}
                type="button"
                onClick={() => moveSlider(-1)}
                disabled={startSlide <= 0}
            >
                <span className="visually-hidden">Предыдущий отзыв</span>
                <svg
                    width="16"
                    height="32"
                    viewBox="0 0 16 32"
            ...
            <button
                ...
                type="button"
                onClick={() => moveSlider(1)}
                disabled={startSlide >= reviews.length - visibleReviewsCount}
            >
                <span className="visually-hidden">Следующий отзыв</span>
                <svg
                    width="17"
                    height="32"
                    viewBox="0 0 17 32"
...
type ReviewCardProps = Omit<IReview, 'id'>;

const ReviewCard: FC<ReviewCardProps> = ({ image, author, city, content }) => {
    return (
        <article className={styles.card}>
            <div className={styles.card__header}>
                ...
                <div className={styles.card__info}>
                    <h3 className={clsx(styles.card__author, 'h6')}>
                        {author}
                    </h3>
                    <address className={styles.card__city}>{city}</address>
                </div>
            </div>
